...
class MediaPlayerService : MediaBrowserServiceCompat() {
    ...
    override fun onCreate() {
        ...
        mediaSessionConnection = MediaSessionConnector(mediaSessionCompat).apply {
            setPlaybackPreparer(AudioMediaPlaybackPreparer())
            setQueueNavigator(MediaQueueNavigator(mediaSessionCompat))
            setPlayer(exoPlayer)
        }
    ...
    override fun onLoadChildren(
        ...
        when (parentId) {
            K.MEDIA_ROOT_ID -> {
                val resultsSend = mediaSource.whenReady { isInitialized ->
                    if (isInitialized) {
    ...
    inner class AudioMediaPlaybackPreparer : MediaSessionConnector.PlaybackPreparer {
        ...
        override fun onPrepareFromMediaId(
            ...
            mediaSource.whenReady {
                ...
                preparedPlayer(
                    metadataCompat = mediaSource.audioMediaMetaData,
    ...
    private fun preparedPlayer(
        metadataCompat: List<MediaMetadataCompat>,
        itemToPlay: MediaMetadataCompat?,
        playWhenReady: Boolean
    ) {
        val indexToPlay = if (currentPlayingMedia == null) 0 else metadataCompat.indexOf(itemToPlay)
